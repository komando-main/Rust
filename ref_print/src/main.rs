#![allow(unused_mut, unused_must_use, static_mut_refs)]
...
//ex) 
//let a:[u8;3] = [1, 2, 3]; 함수인자로 넘겨지는 순간 drop 된다 <<--이거 잘못 이해 한거다 스텍에 생성되어진 데이터들은 암묵적 복사가됨으로 드랍되지 안는다
//실질적으로 힙영역 에서의 생성은 그어떤 데이터든 밸류드 상관 없이 드랍된다
